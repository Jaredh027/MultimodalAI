...
if "messages" not in st.session_state:
    st.session_state.messages = [
            {"role": "assistant", "content": "What style would you like to convert your image to?"}
        ]
...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        # with open("decoded_image.jpeg", "wb") as f:
        #     f.write(image_data)
        # try:
        #     img = Image.open("decoded_image.jpeg")
        #     img.show()  # This should open the image in your default image viewer
        #     print("The image was successfully decoded and appears to be valid.")
        # except IOError:
        #     print("The image file could not be opened. It might be corrupted or improperly encoded.")
        
        # Optionally, prepend the appropriate data URI scheme header for direct use in HTML/Markdown
        mime_type = uploaded_file.type  # Example: "image/jpeg"
        b64_string_with_header = f"data:{mime_type};base64,{b64_string}"
        
        st.success("Image loaded for multimodal RAG Q&A.")
        
        # Store the base64 string in session state for later use
        st.session_state['image_query_base64'] = b64_string_with_header
    
        with st.spinner("Getting image description using NeVA"):
            neva = ChatNVIDIA(model="playground_neva_22b")
            ...
            #b64_string = base64.b64encode(img_thing).decode("utf-8")
            ...
            #                 {
            #                     "type": "image_url",
            #                     "image_url": {"url": f"{b64_string_with_header}"},
            #                 },
            #             ]
            #         )
            #     ]
            ...
            res = neva.invoke(f'Use comma-separated keywords to describe the image (MAX 20 KEYWORDS). No narrative-style prompts.\n<img src="{base64_with_mime_type}" />')

            #res = neva.multimodal_invoke(b64_string, creativity = 0, quality = 5, complexity = 3, verbosity = 0)
...
with placeholder:
    with st.form("chat-form", clear_on_submit=True):
        ...
        with col1:
            cartoon = st.form_submit_button("Cartoon")
        with col2: 
            pop_art = st.form_submit_button("Pop Art")
        with col3: 
            photo_realism = st.form_submit_button("Photo Realism")
    # if submitted:
    #     placeholder.empty()
    #     st.session_state['prompt_value'] = None

def inst_image(img_type):
    with st.spinner("Generating image based on description..."):
        ...
        diff_img = (img_gen | StrOutputParser() | to_pil_img).invoke(st.session_state.image_query + ', ' + img_type)
        st.session_state.diff_img = diff_img

    st.write(st.session_state.image_query + ', ' + img_type)
    st.image(st.session_state.diff_img, caption="Generated Image", use_column_width=True)
    def restart_btn():
        st.experimental_rerun()
    st.button("Restart", on_click=restart_btn)

if cartoon == True:
    inst_image("Cartoon, concept art")
if pop_art == True:
    inst_image("Pop art")
if photo_realism == True:
    inst_image("Photo realism, high definition, sharp focus, studio lighting")
